...
> **Канонический источник старшинства** — [normative/README.md](normative/README.md), раздел «Две оси: порядок присоединения ≠ ступень старшинства». Цепочка ниже в этом файле — порядок **присоединения**, а не ступень старшинства. Ступень старшинства: `корпус v3 → И1 → И2 → И3 → И4 → Б17 → SEAM-PATCH-1 → ERRATA-UNIFIED (высшая)`.
...
> При расхождении об интерфейсе и составе команд побеждает патч Б17 как **более старший слой**: он подшит позже И1–И4 (порядок присоединения `корпус v3 → И1 → И2 → И3 → И4 → Б17 → ERRATA-UNIFIED → SEAM-PATCH-1`) и по ступени старшинства стоит над ними. Над самим Б17 стоят SEAM-PATCH-1 и ERRATA-UNIFIED. Ограничения FSM (Р477: состояний `banned_*` нет) и сроки из И2/И3 патчем Б17 не отменяются.
...
`/refund` разрешён при любом состоянии участника, если платёж был получен, включая `banned_soft` и `banned_hard`, с warning «участник забанен; деньги вернутся, доступ не восстановится»; исключение — `erased`, где реквизитов возврата может не быть технически (долг Б15/Б16 — удержать реквизиты дольше ПДн профиля или отсрочить `erased` до финализации расчётов). **Читать через Р477 (И2): состояний `banned_soft`/`banned_hard` в FSM больше нет** — здесь они означают участника, удержанного связкой `sleeping` + `author_pause` по решению owner; сам возврат состояние участника не меняет.
...
> **Читать через Р477 (И2).** Состояний `banned_soft`/`banned_hard` в FSM нет; описанное ниже поведение гейта доставки применяется к участнику, удержанному связкой `sleeping` + `author_pause`. Коды отказа `send_blocked_hard_ban` и `send_blocked_erased` сохранены дословно как идентификаторы.
...
> **Читать через Р477 (И2).** Колонки `banned_soft` и `banned_hard` в матрицах ниже — исторические: состояний banned_* в FSM больше нет. Значения в этих колонках читаются как поведение для участника, удержанного связкой `sleeping` + `author_pause` по решению owner. Колонки сохранены дословно, чтобы не потерять исходные градации OK/WARN/NO.
